...
- **The Why:** Built screen `1c` from the "Referral Workbench Wireframes" design canvas against the existing frontend conventions rather than the canvas's `modernist` design-system bundle — that bundle is a different aesthetic (Archivo / red / zero-radius) which the wireframe itself does not use. The purple Plena palette the wireframe _does_ use was already tokenised in `globals.css` for screen `1a`, so this screen extends those tokens (`--color-shell`, `--color-success`, `--color-success-tint`, `--color-danger-tint`) instead of introducing a parallel system.
...
- **The Why:** `docs/product_solution_design.md` step 3 specifies a Redis hash `referral:{referral_id}` → `{fileName, extractionSchema}` written at referral-creation time, so the worker can recover both the original file name and the resolved schema in one O(1) lookup — neither survives into the S3 object key (`referrals/{clinicId}/{referralId}.pdf`) or the SQS message built from it. This was previously undesigned; the whole layer was greenfield (no Redis client installed, no cache code anywhere in the repo).
  ...
  - **`onModuleInit` does not crash the app:** initially mirrored `PrismaService` (throw if unreachable at boot), but reconsidered — Redis is a hard dependency of referral _creation_ specifically, not of the whole API (auth and schema management never touch it), so crashing every endpoint over a Redis outage at boot would over-scope the fail-closed policy the user actually asked for. `onModuleInit` now logs a non-fatal warning; the real guarantee is enforced per-call in `setManyReferralsToCache`.
...
- **The Why:** The batch upload flow worked but gave no visible sense of progress across a multi-file batch — the submit button just said "Uploading…" with no percentage, and per-file chip labels were a small 11.5px caption easy to miss. Requested design: the file label made larger/more visible, and the submit button becoming a progress bar (10%, 20%, ... 100%, "documents uploaded" for 500ms, then reset).
  - **Fixed a real bug this surfaced:** `useDirectFileUploads.run` was updating all file statuses in a single batch _after_ `Promise.allSettled` resolved every PUT — meaning progress could never actually animate incrementally, it would jump straight from 0% to 100%. Rewrote it to update each candidate's status the instant _its own_ PUT settles (`Promise.all` with per-item `.then`-style updates inside the mapped callback), which is what makes genuine incremental percentages possible.
...
- **The Why:** The previous progress model (`useDirectFileUploads`) only tracked discrete per-file states (`idle`/`uploading`/`done`/`error`) — the submit button's percentage was derived from _how many files had finished_, not real bytes transferred. Requested: swap to `axios` with `onUploadProgress` for genuine byte-level percentages, and pull all the upload-execution logic (the PUT call, the per-file progress state, the aggregate math) into a single dedicated hook.
...
## v0.0.36 | 2026-08-26 | refactor | ERROR ARCHITECTURE & SAGAS

**Category:** System Architecture  
**Summary:** Standardize tiered error handling hierarchy, implement full-clinic cache-aside with relation lookups, and add compensating sagas for atomic referral batch ingestion.  
**SuggestedCommitMessage:** refactor: implement tiered error hierarchy, full-clinic caching, and compensating saga rollbacks | System Architecture

### 🧠 Logic & Decisions

- **The Why:**
  - **Tiered Error Hierarchy & Translation:** Introduced granular domain (`DomainException`), application (`ApplicationException`), and repository (`RepositoryException`) error classes with explicit enum codes (`DOMAIN_ERROR`, `APPLICATION_ERROR`, `REPOSITORY_ERROR`). Replaced the legacy domain filter with a global `AllExceptionFilter` mapping HTTP statuses and logging internal details, backed by `translateError()` in `ApplicationService` as a centralized error boundary.
  - **Simplified UUID Identifiers:** Replaced redundant single-value object wrappers (`ClinicId`, `ExtractionSchemaId`, `ReferralId`) with native string UUIDs validated directly at aggregate construction boundaries.
  - **Full-Clinic Cache-Aside (`CachedClinic`):** Added `getFullClinic` and `setFullClinic` to `CachingServicePort` and `RedisService`. Refactored `getExtractionSchema` to resolve relation schema IDs via cached `Clinic` models and fetch schema definitions on demand while maintaining tenant isolation.
  - **Compensating Saga for Referral Ingestion:** Refactored `createNewReferralsWithAttachedPresignedUrls` to run atomic Postgres persistence and pipelined Redis caching (`setManyReferralsToCache` + `addToClinicIndexTolerantly`) concurrently via `Promise.allSettled`. Added compensating rollbacks (`rollbackPersistedReferrals`, `rollbackReferralCacheWrites` / `deleteReferralsToCache`) ensuring all-or-nothing consistency across DB and Redis without distributed transactions.
  - **Application Types Clean-up:** Decoupled commands and queries into `apps/workbench-api/src/application/types.ts`.
- **State Change:** Unified error mapping across all layers, optimized clinic schema resolution via Redis cache-aside, and added rollback compensation for multi-system upload failures.
